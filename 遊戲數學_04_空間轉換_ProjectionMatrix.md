...
在進入矩陣推導前，我們要先認識兩個關鍵的空間：**Clip Space (裁剪空間)** 與 **Normalized Device Coordinates (NDC，標準化設備座標)**。

透過相機觀看 3D 世界，相機能捕捉到的範圍稱為**視錐體 Frustum**，我們要把這個 Frustum 裡的所有東西包裝進一個 $[-1, 1]$ 的「標準正方體」中，如此 GPU 才方便把畫面畫到 2D 螢幕上。

這個轉換分為 2 個階段（也是初學者最容易混淆的地方）：
...
2. **NDC**：裁剪完畢後，GPU 會自動執行透視除法 **(Perspective Division)**，將所有頂點的 $x, y, z$ 都除以 $w$。
