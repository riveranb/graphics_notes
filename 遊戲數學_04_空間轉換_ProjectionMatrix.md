...
## Clip Space、NDC 與透視除法

在進入矩陣推導前，我們要先認識兩個關鍵的空間：**Clip Space (裁剪空間)** 與 **NDC (標準化設備座標)**。

為什麼需要這兩個空間？想像你正透過相機觀看 3D 世界，相機能捕捉到的範圍是一個特定的形狀（稱為**視錐體 Frustum**）。我們要把這個形狀裡的所有東西，最終都裝進一個 $[-1, 1]$ 的「標準正方體」中，這樣 GPU 才方便把畫面畫到 2D 螢幕上。

這個轉換分為兩個階段（也是初學者最容易混淆的地方）：

1. **Clip Space**：頂點座標乘上 Projection Matrix 後，就會來到 Clip Space。此時的座標是一個 4D 的齊次座標 $(x, y, z, w)$。GPU 在這個階段會進行 **Clipping (裁剪)**，把超出身邊界範圍的三角形切掉。
2. **NDC**：裁剪完畢後，GPU 會自動執行一個動作叫做 **Perspective Division (透視除法)**。它會把所有的 $x, y, z$ 都除以 $w$。
